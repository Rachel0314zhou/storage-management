...
<%
    Map<String,Object> c =
            (Map<String,Object>)request.getAttribute("customer");
%>
...
<div class="container">
    ...
    <form action="<%=request.getContextPath()%>/customer?action=update" method="post">
        <input type="hidden" name="customer_id" value="<%=c.get("customer_id")%>">

        <div class="form-row">
            <label>客户名称</label>
            <input name="customer_name" value="<%=c.get("customer_name")%>">
        </div>
        <div class="form-row">
            <label>联系方式</label>
            <input name="phone" value="<%=c.get("phone")%>">
        </div>
        <div class="form-row">
            <label>地址</label>
            <input name="address" value="<%=c.get("address")%>">
        </div>

        <div class="actions">
            <button class="btn btn-green" type="submit">保存修改</button>
            <a class="btn btn-gray" href="<%=request.getContextPath()%>/customer">返回客户列表</a>
        </div>
    </form>
</div>

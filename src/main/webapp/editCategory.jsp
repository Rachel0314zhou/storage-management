...
<%
    Map<String,Object> c =
            (Map<String,Object>)request.getAttribute("category");
%>
...
<div class="container">
    ...
    <form action="<%=request.getContextPath()%>/category?action=update" method="post">
        <input type="hidden" name="category_id" value="<%=c.get("category_id")%>">

        <div class="form-row">
            <label>分类名称</label>
            <input name="category_name" value="<%=c.get("category_name")%>">
        </div>
        <div class="form-row">
            <label>分类说明</label>
            <textarea name="description"><%=c.get("description") == null ? "" : c.get("description")%></textarea>
        </div>

        <div class="actions">
            <button class="btn btn-green" type="submit">保存修改</button>
            <a class="btn btn-gray" href="<%=request.getContextPath()%>/category">返回分类列表</a>
        </div>
    </form>
</div>
